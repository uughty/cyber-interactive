...
if (SMTP_PORT === 465 && !SMTP_SECURE) {
    console.error("SMTP_PORT is 465, but SMTP_SECURE is not true.");
    process.exit(1);
}
if (SMTP_PORT !== 465 && SMTP_SECURE) {
    console.error("SMTP_SECURE should normally be false when SMTP_PORT is 587 or 25.");
    process.exit(1);
}
...
app.post("/api/security-assessment", formLimiter, async (request, response) => {
    console.log("Received security assessment request from:", request.ip);
    try {
        const data = getFormData(request.body);
...
`;
        const emailInfo = await transporter.sendMail({
            ...
            to: RECIPIENTS,
            replyTo: data.email,
            subject: `New Security Assessment Request${data.company ? ` - ${data.company}` : ""}`,
            text: textMessage,
            html: htmlMessage,
        });
        ...
        return response.status(500).json({
            error: "The email could not be sent. Check the API logs.",
        });
    }
});
...
app.get("/{*splat}", (_request, response) => {
    response.sendFile(path.join(DIST_DIR, "index.html"), (error) => {
        if (error) {
...
async function startServer() {
    try {
        ...
        app.listen(PORT, "0.0.0.0", () => {
            console.log(`CyberTrove API running on port ${PORT}`);
        });
    }
    catch (error) {
        console.error("SMTP verification failed.");
        console.error(error);
        console.error("Check SMTP_HOST, SMTP_PORT, SMTP_SECURE, SMTP_USER, SMTP_PASSWORD, and SMTP_FROM.");
        process.exit(1);
    }
}
startServer();
